// Import JSZip (include JSZip library in your HTML)
...
function replaceInlineCode(text) {
  ...
  text = text.replace(/```(\w*)\n([\s\S]*?)```/g,
    (match, group1, group2) => `<pre class="code-block ${group1}">${escapeHtml(group2)}</pre>`);

  // Handle inline code with single backticks
  text = text.replace(/`([^`]+)`/g,
    (match, group1) => `<code class="inline-code">${escapeHtml(group1)}</code>`);
  ...
  function processNestedNumberedList(text) {
    ...
    function createListHtml(lst) {
      return lst.items.map(item => {
    ...
    for (const line of lines) {
      ...
      const [_, indent, number, content] = numberMatch;
...
function replaceArtifactTags(inputText, artifactPanels, printArtifacts = false) {
  artifactCounter = 0; // Reset counter for each conversation
  ...
  function replaceMatch(match, content, closingTag) {
    artifactCounter++;
    const fullMatch = match[0];
    const hasClosingTag = closingTag === "</antArtifact>";
  ...
  return inputText.replace(pattern, replaceMatch);
}

function generateHtml(jsonData, printArtifacts = false) {
  try {
    const parsed = JSON.parse(jsonData);
    if (!parsed || !parsed.chat_messages) {
      return "";
    }
    ...
    for (const message of parsed.chat_messages) {
      ...
      for (const content of message.content) {
        if (content.type === "tool_use" && content.name === "repl") {
          ...
          messageContent += escapeHtml(JSON.stringify(content));
        }
      }
      ...
      const messageClass = message.sender.toLowerCase();
...
function generateToc(tocEntries) {
  ...
  for (const [title, filename] of tocEntries) {
    html += `<li><a href="${filename}">${escapeHtml(title)}</a></li>\n`;
  }
...
async function processClaudeExport(jsonData) {
  const data = JSON.parse(jsonData);
  if (!Array.isArray(data)) {
    throw new Error("The JSON data should be a list of conversations.");
  }
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (typeof conversation !== 'object' || conversation === null) {
      console.warn("Warning: Found a non-dictionary element in the conversation list. Skipping.");
      continue;
    }
    ...
    if (
      ...
      (conversation.name && typeof conversation.name === 'string' && conversation.name === "")
    ) {
      deletedCount++;
      continue;
    }
    ...
    if (!conversation.name || typeof conversation.name !== 'string') {
      console.warn("Warning: Conversation missing 'name' or 'name' is not a string. Using a generic filename.");
      baseFilename = `conversation_${i + 1}`;
      conversationName = baseFilename;
    } else {
      conversationName = conversation.name;
      baseFilename = conversationName.replace(/[^a-zA-Z0-9._\- ]/g, '').trim();
      if (!baseFilename) {
        baseFilename = `conversation_${i + 1}`;
      }
    }
    ...
    try {
      const htmlOutput = generateHtml(JSON.stringify(conversation));
      zip.file(filename, htmlOutput);
      tocEntries.push([conversationName, filename]);
      console.log(`Successfully wrote conversation to '${filename}'`);
    } catch (error) {
  ...
  console.log("Successfully wrote table of contents to 'index.html'");

  console.log(`Found and skipped ${deletedCount} deleted (empty) conversations.`);
...
// Example usage
// processClaudeExport(yourJsonString);